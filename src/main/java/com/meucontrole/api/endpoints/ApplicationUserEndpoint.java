...
public class ApplicationUserEndpoint {
    ...
    @POST
    public Response newUser(NewUserDTO newUserDTO) throws BadRequestException {
        User user = applicationUserService.newUser(newUserDTO.convertToObject());
        return Response.created(UriBuilder.fromPath("/user/{id}").build(user.getId())).build();
    }

    @GET
    @Path("{id}")
    public Response getUser(@PathParam("id") String id) throws NotFoundException {
        return Response.ok(UserDTO.getDTOObject(applicationUserService.getUserById(id))).build();
    }
}
